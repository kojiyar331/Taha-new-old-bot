...
module.exports.run = async function({ api, event }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		...
		const fs = require("fs");
		return api.sendMessage("ASSALAMUALAIKUM EVERYONE 🙋‍♂️", event.threadID, () => api.sendMessage({body:`🌺𝐓𝐀𝐇𝐀-𝐁𝐀𝐁𝐔  𝐁𝐎𝐓🦋🌺 𝐂𝐎𝐍𝐍𝐄𝐂𝐓𝐄𝐃«

𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐛𝐲𝐞 𝐌𝐑.. 𝐓𝐀𝐇𝐀.𝐁𝐀𝐁𝐔☘️
<------------------------------>  
...
⚠️Wp: +9203474771404`, attachment: fs.createReadStream(__dirname + "/cache/join.jpg")} ,threadID));
	}
	else {
		try {
